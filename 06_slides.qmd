...
2. Robustness / Falsification samples (measurement challenges)
    - Occupation mismeasurement
      ...
      - 2016 TEER–highest attainment mismatch  
...
- At low $\varepsilon$ the model puts too much weight on cost minimizing moves,
and too little weight on random moves: these "false negatives" are heavily penalized by KL.
- The binary cost matrix is more resistant to making false negative mistakes, and wins at low temperatures.
...
- Segment by destination gating (entry tightness)
    ...
    - Weakly gated → 100% skill distance  
    - Moderately gated → 100% hierarchical distance  
    - Highly gated → 100% binary distance (constrained mobility)
...
- Pipeline concentration: $$KL(p \parallel p_0)= \sum_{k} p_k \, \log\!\left(\frac{p_k}{p_{0k}}\right)$$ where $p_0$ is the overall workforce education distribution  
...
- If gating is a continuum, using sextiles rather than tertiles would likely show a gradient-plateau-gradient structure
...
-   Labour shortages in different segments likely require different adjustment mechanisms
    ...
    -   Medium gating: direct workers towards "first rung" of career ladders
